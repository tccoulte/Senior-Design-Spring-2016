...
def SNR_p():
    ...
    while received_message and phone.get_amplitude() >= amp_min:
        ...
        message_to_send = "Fuck this project because"
        phone.send_message(message_to_send, 20)
        time.sleep(.8)
        phone.receive_message(time_out = 5)
        time.sleep(1.5)
        if phone.get_state() == class_state.RECEIVED:
            if phone.get_message() == "The message was received":
                phone.set_amplitude(decrement_ampl(phone.get_amplitude(), amp_decrement))
...
def decrement_ampl(ampl, amount = .05):
    if ampl <= .11:
        ampl = ampl - .01
    else:
        ampl = ampl - amount
